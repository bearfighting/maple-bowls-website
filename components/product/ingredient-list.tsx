...
import type { ResolvedProductIngredient } from "@/lib/content";
...
export function IngredientList({
  ...
  ingredients: ReadonlyArray<ResolvedProductIngredient>;
  locale: Locale;
  unknownLabel: string;
}) {
  ...
  return (
    <ul className="space-y-2">
      {ingredients.map((ingredient, index) => (
        <li
          key={getLocalizedText(ingredient.item.name, locale) + "-" + index}
        ...
        >
          {ingredient.ingredient ? (
            ...
            >
              {getLocalizedText(ingredient.item.name, locale)}
            </Link>
          ) : (
            <span>{getLocalizedText(ingredient.item.name, locale)}</span>
          )}
        </li>
      ))}
    </ul>
