  ...
  return (
    <SectionContainer>
      <Breadcrumb
        ...
        items={[{ label: navigation("home"), href: "/" }, { label: t("nutritionTitle") }]}
      />
        ...
        {topics.length > 0 ? (
          <div className="mt-6 grid gap-5 md:grid-cols-3">
            {topics.map((topic) => (
              <div key={topic.id}>
                <ArticleCard
                  href={"/nutrition-guide/" + topic.slug}
                  title={getLocalizedText(topic.title, locale)}
                  summary={getLocalizedText(topic.summary, locale)}
                  label={t("topic")}
                />
                <div className="mt-2">
                  <ContentStatusBadge
                    status={topic.status}
                    labels={t.raw("status") as Record<"draft" | "verified", string>}
                  />
                </div>
              </div>
            ))}
          ...
          <p className="mt-6 rounded-xl bg-muted p-4 text-sm text-muted-foreground">{t("empty")}</p>
        )}
      </section>
      <section className="mt-12">
        ...
        {guides.length > 0 ? (
          <div className="mt-6 grid gap-5 md:grid-cols-2">
            {guides.map((guide) => (
              <div key={guide.id}>
                <ArticleCard
                  href={"/nutrition-guide/" + guide.slug}
                  title={getLocalizedText(guide.title, locale)}
                  summary={getLocalizedText(guide.summary, locale)}
                  label={t("guide")}
                />
                <div className="mt-2">
                  <ContentStatusBadge
