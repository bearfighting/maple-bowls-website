    ...
    <div>
      ...
      <section className="border-y border-border/70 bg-card">
        <div className="mx-auto grid w-full max-w-7xl gap-4 px-4 py-8 sm:grid-cols-3 sm:px-6 lg:px-8">
          {[
            ...
            <Card key={title as string} className="flex items-start gap-4 border-transparent bg-background p-5 shadow-none">
              <span aria-hidden className="text-2xl">{icon}</span>
              <div><h2 className="font-display text-lg font-bold">{title}</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">{description}</p></div>
            </Card>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div><p className="text-sm font-bold uppercase tracking-[0.18em] text-accent">Maple Bowl</p><h2 className="mt-2 font-display text-3xl font-bold sm:text-4xl">{t("explore")}</h2></div>
