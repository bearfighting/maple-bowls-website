...
import { getBrandBySlug, getBrands, getLocalizedText, getProductsByBrand } from "@/lib/content";
...
export default async function BrandDetailPage({ params }: { params: Params }) {
  ...
  return (
    <SectionContainer>
      <Breadcrumb label={t("brand")} items={[{ label: "Maple Bowl", href: "/" }, { label: t("brandsTitle"), href: "/brands" }, { label: getLocalizedText(brand.name, locale) }]} />
      <p className="text-sm font-bold uppercase tracking-[0.18em] text-accent">{t("brand")}</p>
      <h1 className="mt-3 font-display text-4xl font-bold tracking-tight sm:text-5xl">{getLocalizedText(brand.name, locale)}</h1>
      <p className="mt-5 max-w-2xl text-lg leading-8 text-muted-foreground">{getLocalizedText(brand.description, locale)}</p>
      <section className="mt-12">
        <h2 className="font-display text-3xl font-bold">{t("productsTitle")}</h2>
        <div className="mt-6 grid gap-5 md:grid-cols-2">
          {products.map((item) => <ProductCard key={item.id} product={item} brandName={getLocalizedText(brand.name, locale)} locale={locale} labels={{ species, foodType }} />)}
        </div>
      </section>
    </SectionContainer>
  );
