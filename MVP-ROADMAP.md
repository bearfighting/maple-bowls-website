...
- [x] 新增一个 Brand/Product/Ingredient 主要通过添加数据完成，而不是复制页面组件。
- [x] 目录、详情页和关联链接使用同一套实体数据。
- [x] 空数据和无结果状态有明确文案和返回路径。
- [x] 页面层没有大段重复的产品/品牌展示逻辑。
- [x] 内容关联错误能在校验阶段被定位，不会在页面渲染时静默丢失。
- [x] 新增内容只需更新数据和必要的校验，不需要复制关联拼装逻辑。
- [x] 所有 draft 内容在页面上有清晰状态提示，开放字段不会产生空白 UI 标签。
